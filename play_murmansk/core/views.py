...
def get_comments_count_for_object(model_object, object_id):
    ...
    comments_count = CommonComment.objects.filter(
        content_type=object_content_type, object_id=object_item.id
    ).count()
...
class PostInfoSaturation:
    ...
    def get_comments_for_object(self, model_object, object_id):
        ...
        comments = CommonComment.objects.filter(
            content_type=post_content_type, object_id=post_item.id
        )
        return comments
    ...
    def get_context_data(self):
        ...
        context["form"] = CommonCommentForm(
            author=self.request.user, content_object=self.object
        )
...
class PostMethodCommentForm:
    ...
    def post(self, request, *args, **kwargs):
        ...
        form = CommonCommentForm(
            request.POST, author=request.user, content_object=object_item
        )
        if form.is_valid():
            form.save()
            # TODO исправить костыль с редиректом!
            return redirect(
                f"{self.context_object_name}_detail", pk=object_item.id
