...
from forum.models import (ForumMessage, ForumSection, ForumSubsection,
                          ForumTopic)
...
ForumMessage.objects.create(
    topic=topic1, author=user2, content="Первое сообщение в теме 1"
)
...
comment_ads_1 = CommonComment.objects.create(
    content_object=ad1, author=user2, content="Сколько стоит плойка?"
)
comment_ads_2 = CommonComment.objects.create(
    content_object=ad2, author=user2, content="Сколько стоит бокс?"
)
comment_ads_3 = CommonComment.objects.create(
    content_object=ad3, author=user2, content="Азаза, кому нужен этот обрубок?"
)
comment_news_1 = CommonComment.objects.create(
    content_object=news1, author=user2, content="Вот это новости!"
)
comment_news_2 = CommonComment.objects.create(
    content_object=news2, author=user2, content="Сони уже не те!"
)
comment_news_3 = CommonComment.objects.create(
    content_object=news3, author=user2, content="Майкрософт уже не те!"
)
comment_review_1 = CommonComment.objects.create(
    content_object=review1, author=user2, content="Классный обзор!"
)
comment_review_2 = CommonComment.objects.create(
    content_object=review2, author=user2, content="Не согласен, игра крутая!"
)
comment_review_3 = CommonComment.objects.create(
    content_object=review3, author=user2, content="Супер, прочёл на одном дыхании!"
)
