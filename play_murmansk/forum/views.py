...
class PostMethodCommentForm:
    ...
    def post(self, request, *args, **kwargs):
        ...
        form = ForumMessageForm(
            request.POST, author=request.user, topic_object=object_item
        )
        if form.is_valid():
            form.save()
            ...
            return redirect(
                "forum_topic_detail", pk=object_item.id
...
class ForumSectionListView(ListView):
    ...
    def get_queryset(self):
        ...
        last_message_subquery = (
            ForumMessage.objects.filter(topic__subsection=OuterRef("pk"))
            .order_by("-created_at")
            .values("created_at", "topic__title", "author__username")[:1]
        )
        ...
        subsections = ForumSubsection.objects.annotate(
            last_message_created_at=Subquery(
                last_message_subquery.values("created_at")
            ),
            last_message_topic_title=Subquery(
                last_message_subquery.values("topic__title")
            ),
            last_message_author_username=Subquery(
                last_message_subquery.values("author__username")
            ),
        ).order_by(
            "id"
...
class ForumTopicListView(ListView):
    ...
    def get_queryset(self):
        ...
        last_message_subquery = (
            ForumMessage.objects.filter(topic=OuterRef("pk"))
            .order_by("-created_at")
            .values("author__username", "created_at")[:1]
        )

        queryset = queryset.annotate(
            last_message_author=Subquery(
                last_message_subquery.values("author__username")
            ),
    ...
    def get_context_data(self, **kwargs):
        ...
        context["subsection"] = ForumSubsection.objects.filter(
            id=self.kwargs.get("subsection_id")
        ).first()
...
class ForumTopicDetailView(DetailView, PostMethodCommentForm):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["form"] = ForumMessageForm(
            author=self.request.user, topic_object=self.object
        )
